...
# option
flags.DEFINE_string('snapshot',  default=None, help='filename of model snapshot [default: None]')
flags.DEFINE_string('predict',  default=None, help='predict the sentence given')
flags.DEFINE_bool('test',  default=False, help='train or test')
...
def main(argv):
    ...
    train_iter = pickle.load( \
        open(os.path.join(FLAGS.data_path, FLAGS.dataset + f'_train_iter_fake{FLAGS.noise_rate}.pkl'), mode='rb'))
    dev_iter = pickle.load( \
        open(os.path.join(FLAGS.data_path, FLAGS.dataset + f'_dev_iter_fake{FLAGS.noise_rate}.pkl'), mode='rb'))
    test_iter = pickle.load( \
        open(os.path.join(FLAGS.data_path, FLAGS.dataset + f'_test_iter_fake{FLAGS.noise_rate}.pkl'), mode='rb'))
    ...
    train.train(train_iter, test_iter, cnn, FLAGS)
    print("test")
